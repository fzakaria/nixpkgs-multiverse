#!/usr/bin/env bash
# The cache key the per-revision extractions are addressed by, in one place.
#
# The key is the hash of everything that decides what the extractor produces:
# the extractor itself, and the package-set list it walks. Editing either has
...
# passed in, since under `nix run` it is a store path the caller knows and this
# file cannot derive.
